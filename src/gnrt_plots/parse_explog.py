...
def extract_exps(logs, model, appr):
    ...
    for conf in stats_dict:
        bg, xtype = conf
        saved_dir = f'../stats/{"apriori_" if appr != "xcon" else ""}expls/{model}'
...
def grt_scatter(files, model, xtype, appr):
    ...
    for key in keys:
        ...
        saved_dir = '../plots/{0}{1}'.format('apriori/' if appr != 'xcon' else '', model)
        if not os.path.isdir(saved_dir):
            os.makedirs(saved_dir)
...
def grt_size_scatter(files, model, xtype, appr):
    ...
    for key in keys:
        ...
        #maxkey = 15 if model in ('dl', 'bt') else 17
    ...
    for dt in change:
        for conf in change[dt]:
            ...
            if change[dt][conf] % 1 == 0:
                ...
                change[dt][conf] = str(change[dt][conf])
...
def expls_table(change, datasets):
    ...
    head = ['\\begin{table}[ht!]',
            '\\centering',
            '\\caption{Change of Average Minimum Explanation Size in DLs, BTs and BNNs}',
            '\\label{tab:exp}',
            '\\scalebox{0.92}{',
...
if __name__ == '__main__':
    ...
    appr = sys.argv[-1]
    for root, dirs, files in os.walk('../{0}logs'.format('apriori_' if appr != 'xcon' else '')):
        for file in files:
            if '/rextract' in root:
                continue
    ...
    for m in m2logs:
        json_files = glob.glob('../stats/{0}expls/{1}/*.json'.format('apriori_' if appr != 'xcon' else '',
                                                               m))
        bg = ['ori', 'size5']
        xtypes = ['abd', 'con']
        files[m] = {}
