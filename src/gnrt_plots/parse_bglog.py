...
def rextractrtime(logs, appr):
    ...
    for conf in configs:
        ...
        fdir = '../stats/{0}rtime/rextract'.format('apriori_' if appr != 'xcon' else '')
...
def get_nof_rules(logs):
    ...
    for conf in rdict:
        ...
        for e in ["prog_alias", "prog_args", "program"]:
            ...
            rdict[conf]['preamble'][e] = 'apriori' if 'apriori_' in logs[0] else 'xcon'
            avg_rdict[conf]['preamble'][e] = 'apriori' if 'apriori_' in logs[0] else 'xcon'

        fdir = '../stats/{0}nof_rules'.format('apriori_' if 'apriori_' in logs[0] else '')
        if not os.path.exists(fdir):
            os.makedirs(fdir)
...
if __name__ == '__main__':
    ...
    #pasrse logs
    for appr in ['apriori', 'xcon']:
        logs = glob.glob('../{0}logs/rextract/*.log'.format('apriori_' if appr != 'xcon' else ''))
    ...
    for avg in [True]:
        files = a_avg2files[avg]
        ori_files = ori_avg2files[avg]

        #for i in range(1):
        rtimescatter([ori_files[0], files[0]])
        #rtimecactus([ori_files[0], files[0]], avg)
    ...
    for a_name in apriori_nof_rules:

        # apriori info
        with open(a_name, 'r') as f:
            apriori_info = json.load(f)

        # xcon info
        name = a_name.replace('apriori_nof_rules/', 'nof_rules/')
        with open(name, 'r') as f:
            info = json.load(f)

        dtnames = list(apriori_info['stats'].keys())
        for dt in dtnames:
            apriori_status = apriori_info['stats'][dt]['status']
            if not apriori_status:
                del apriori_info['stats'][dt]
                del info['stats'][dt]

        assert len(apriori_info['stats'].keys()) == len(info['stats'])

        a_new_dir = a_name[: a_name.rfind('/')] + '/allsolved'
        new_dir = name[: name.rfind('/')] + '/allsolved'

        if not os.path.exists(a_new_dir):
            os.makedirs(a_new_dir)

        if not os.path.exists(new_dir):
            os.makedirs(new_dir)

        with open(f'{a_new_dir}/{a_name.rsplit("/")[-1]}', 'w') as f:
            json.dump(apriori_info, f, indent=4)
